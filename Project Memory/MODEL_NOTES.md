...
- Dedicated plain-number KPI alias measures were tested as a safer way to force two-decimal rendering on top cards, but in the live report they proved less reliable than the original measures. The safer fallback is to keep the cards bound to the original SAP-backed measures and force non-compact rendering at the visual level.
- Card caption text should not rely on helper measure names. When the live visual still shows internal aliases such as `KPI Plain`, the safer report-side fix is to hide the category label and drive the user-facing caption through an explicit visual title.
- More specifically for these `cardVisual` objects: `categoryLabels.show = false` was not sufficient in live Desktop rendering. Screenshot validation showed the helper caption still rendered under the title. For this project, treat that setting as unreliable and instead suppress the helper caption visually with tiny white text if the title must carry the business-facing name.
