...
if ($replaced) {
    ...
    Write-Host "SalesFact query v3 applied:"
    Write-Host "  - Sales = LineTotal with proportional header discount allocation"
    Write-Host "  - COGS = StockValue for items (actual GL posting), 0 for non-items"
} else {
